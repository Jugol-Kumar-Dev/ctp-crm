<template>
    ...
    <UpperQuotation :subtotal="totalPrice" :clients="props.clients" :errors="props.errors"
                    @handleQuotation="saveQuotation">
                        ...
                        <div class="row">
                            <div class="col-12 d-flex product-details-border position-relative pe-0">
                                ...
                                <div class="row w-100 pe-lg-0 pe-1 py-2">
                                    <div :class="formData.items[index].service ? 'col-md-7' : 'col-md-12'">
                                        <div class="input-group"
                                            ...
                                            <div class="d-flex w-100">
                                                ...
                                                <button v-if="formData.items[index].service != null"
                                                        class="btn border-1 border-start-0 close-select"
                                        ...
                                        <div v-if="formData.items[index].activeTab === 'custom'">
                                            ...
                                            <div class="d-flex gap-2 my-1 align-items-center">
                                                <div class="d-flex">
                                                    <button @click="minusCustomQty(index)"
                                                            class="btn btn-icon btn-primary rounded-0">
                                                        <vue-feather type="minus"/>
                                                    </button>
                                                    <input type="text" class="form-control rounded-0"
                                                           :value="formData.items[index].customItem?.qty"
                                                           placeholder="change here qty and price">
                                                    <button @click="plusCustomQty(index)"
                                                            class="btn btn-icon btn-primary rounded-0">
                                                ...
                                                <div>
                                                    ...
                                                    <input type="text" id="customItemPrice"
                                                           ...
                                                           v-model="formData.items[index].customItem.price"
                                                           @input="changeCustomSubTotal(index)"
                                                           placeholder="e.g total price">
                                                </div>
                                                <div>
                                                    <label for="customItemSubTotal">Sub Total</label>
                                                    <input type="text" id="customItemSubTotal" disabled
                                                           class="form-control rounded-0"
                                                           v-model="formData.items[index].customItem.subTotal"
                                                           placeholder="e.g total price">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="showContentQuotation border p-2"
                                             v-if="formData.items[index].activeTab === 'feature'"
                                             v-for="(fes, j)  in formData.items[index].checkFeatrueds">
                                            ...
                                            <div class="d-flex flex-column gap-1 position-relative">
                                                <vue-feather type="x" class="packDelete" size="15"
                                                             @click="itemRemove(index, j)"/>
                                                <div class="d-flex gap-1">
                                                    <div>
                                                        <label for="featureTitle">Title</label>
                                                        <input type="text" id="featureTitle"
                                                               class="form-control rounded-0"
                                                               v-model="formData.items[index].checkFeatrueds[j].name">

                                                    </div>
                                                    <div>
                                                        <label for="featurePrice">Package Price</label>
                                                        <input type="text" id="featurePrice"
                                                               class="form-control rounded-0"
                                                               v-model="formData.items[index].checkFeatrueds[j].price"
                                                               @input="changeFeatureSubTotal(index, j)"
                                                               placeholder="e.g total price">
                                                    </div>
                                                </div>
                                                <div class="d-flex align-items-center gap-1">
                                                    <div class="d-flex">
                                                        <button @click="qtyMinus(index, j)"
                                                                class="btn btn-icon btn-primary rounded-0">
                                                            <vue-feather type="minus"/>
                                                        </button>
                                                        <input type="text" class="form-control rounded-0"
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="text" id="featureSubTotal"
                                                               class="form-control rounded-0"
                                                               v-model="formData.items[index].checkFeatrueds[j].subTotal"
                                                               disabled placeholder="e.g subtotal">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div v-if="formData.items[index].activeTab === 'package'"
                                             v-for="(packs, k) in formData.items[index].checkPackages">
                                            <div class="border position-relative">
                                                <textarea class="form-control rounded-0" rows="5">{{ packs?.descriptions }}</textarea>
                                                <vue-feather type="x" class="packDelete" size="15"
                                                             @click="packItemRemove(index, k)"/>
                                            </div>
                                            <div class="d-flex gap-2 my-1 align-items-center">
                                                <div class="d-flex">
                                                    <button @click="minusPackageQty(index, k)"
                                                            class="btn btn-icon btn-primary rounded-0">
                                                        <vue-feather type="minus"/>
                                                    </button>
                                                    <input type="text" class="form-control rounded-0"
                                                           :value="packs?.qty" placeholder="change here qty and price">
                                                    <button @click="plusPackageQty(index, k)"
                                                            class="btn btn-icon btn-primary rounded-0">
                                                        <vue-feather type="plus"/>
                                                    </button>
                                                </div>
                                                <div>
                                                    ...
                                                    <input type="text" id="packagePrice" class="form-control rounded-0"
                                                           v-model="formData.items[index].checkPackages[k].price"
                                                           @input="changeSubTotal(index, k)"
                                                           placeholder="e.g total price">
                                                </div>
                                                <div>
                                                    <label for="subtotalPackage">Sub Total</label>
                                                    <input type="text" id="subtotalPackage" disabled
                                                           class="form-control rounded-0"
                                                           v-model="formData.items[index].checkPackages[k].subTotal"
                                                           placeholder="e.g total price">
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="btn-group" v-if="formData.items[index].service != null">
                                            <button @click="getPlatItems(index, 'custom')"
                                                    :class="`customClass-${index}`"
                                                    class="border-0 btn btn-primary btn-sm">Custom
                                            </button>
                                            <button @click="getPlatItems(index, 'feature')"
                                                    :class="`featuredClass-${index}`" class="border-0 btn btn-sm">
                                                Featureds
                                            </button>
                                            <button @click="getPlatItems(index, 'package')"
                                                    :class="`packageClass-${index}`" class="border-0 btn btn-sm">
                                                Packages
                                            </button>
                                        </div>
                                        <div class="featruedOverflowStyle"
                                             v-if="formData.items[index].activeTab === 'feature'">
                                            <div class="input-group"
                                                 v-for="(fes, i) in  formData.items[index].features">
                                                <div class="form-check form-check-primary">
                                                    <input type="checkbox"
                                                           v-model="formData.items[index].checkFeatrueds" :value="fes"
                                                           class="form-check-input" :id="`featureCheck-${index}-${i}`">
                                                    <label class="form-check-label" :for="`featureCheck-${index}-${i}`">{{
                                                            fes.name
                                                        }} ({{ fes.price }})</label>
                                                </div>
                                            </div>
                                        ...
                                        <div class="featruedOverflowStyle"
                                             v-if="formData.items[index].activeTab === 'package'">
                                            <div class="input-group"
                                                 v-for="(pack, i)  in  formData.items[index].packages">
                                                <div class="form-check form-check-primary">
                                                    <input type="checkbox" v-model="formData.items[index].checkPackages"
                                                           :value="pack" class="form-check-input"
                                                           :id="`packageCheck-${index}-${i}`">
                                                    <label class="form-check-label" :for="`packageCheck-${index}-${i}`">{{
                                                            pack.name
                                                        }} ({{ pack.price }})</label>
                                                </div>
                                            </div>
                                ...
                                <div
                                    ...
                                    <vue-feather type="x" class="cursor-pointer font-medium-3"
                                                 @click="removeItem(index)"></vue-feather>
                                    <!--                                    <div class="dropdown">
...
const saveQuotation = (events) => {
    ...
    router.post(props.main_url, {...formData, ...events}, {
        ...
        onSuccess: () => {
            $toast.success('Quotation Created Successfully Done...')
        },
        onError: () => {
